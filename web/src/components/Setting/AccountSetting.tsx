...
import { selectGradeList, selectUser } from '../../store/globalSlice'
        ...
        <div className="left">
          <div className="word">
            ...
            <div className="content">
              <div className='free'><span>{getGradeName()}</span></div>
              <div className='content-word'>{user.endTime ? '有效期至:' + user.endTime : '会员已过期'}</div>
            </div>
          </div>
        </div>

        <Button type="default" onClick={()=>setIsModalOpen(true)}>立即升级</Button>
      </div>
    </UIROOT>
  )
}
