...
#使用说明，用来提示输入参数
usage() {
 echo "Usage: sh cms-admin.sh [start|stop|restart|status]"
 exit 1
}
...
is_exist(){
 ...
 pid=`ps -ef|grep $WORKFLOW_NAME|grep -v grep|awk '{print $2}' `
 #如果不存在返回1，存在返回0
 if [ -z "${pid}" ]; then
 return 1
 else
...
stop(){
 ...
 echo "${WORKFLOW_NAME} is not running"
 fi
}
...
status(){
 is_exist
 if [ $? -eq "0" ]; then
 echo "${WORKFLOW_NAME} is running. Pid is ${pid}"
 else
 echo "${WORKFLOW_NAME} is NOT running."
 fi
}
